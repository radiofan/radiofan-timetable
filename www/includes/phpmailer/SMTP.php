...
class SMTP{
	...
	public function connect($host, $port = null, $timeout = 30, $options = array()){
		...
		$this->edebug("Connection: opening to $host:$port, timeout=$timeout, options=".(sizeof($options) > 0 ? var_export($options, true) : 'array()'), self::DEBUG_CONNECTION);
	...
	public function recipient($address, $dsn = ''){
		...
		return $this->sendCommand('RCPT TO', $rcpt, array(250, 251));
	}
	...
	protected function sendCommand($command, $commandstring, $expect){
		...
		if(preg_match('/^([\d]{3})[ -](?:([\d]\\.[\d]\\.[\d]{1,2}) )?/', $this->last_reply, $matches)){
			$code = (int) $matches[1];
			$code_ex = (sizeof($matches) > 2 ? $matches[2] : null);
	...
	public function client_send($data, $command = ''){
		...
		if(self::DEBUG_LOWLEVEL > $this->do_debug && in_array($command, array(
				'User & Password',
				'Username',
				'Password'
			), true)){
