...
class PHPMailer{
	...
	protected function mailSend($header, $body){
		...
		$result = false;
		if($this->SingleTo && sizeof($toArr) > 1){
	...
	protected function smtpSend($header, $body){
		...
		if((sizeof($this->all_recipients) > sizeof($bad_rcpt)) && !$this->smtp->data($header.$body)){
		...
		foreach($callbacks as $cb){
			$this->doCallback($cb['issent'], array($cb['to']), array(), array(), $this->Subject, $body, $this->From, array('smtp_transaction_id' => $smtp_transaction_id));
		}
		
		//Create error message for any bad addresses
		if(sizeof($bad_rcpt) > 0){
			$errstr = '';
	...
	public function createHeader(){
		...
		if(('sendmail' === $this->Mailer || 'qmail' === $this->Mailer || 'mail' === $this->Mailer) && sizeof($this->bcc) > 0){
	...
	protected function validateEncoding($encoding){
		return in_array($encoding, array(
				self::ENCODING_7BIT,
		...
		if(array_key_exists(2, $images)){
			...
			foreach($images[2] as $imgindex => $url){
				...
				if(preg_match('#^data:(image/(?:jpe?g|gif|png));?(base64)?,(.+)#', $url, $match)){
					if(sizeof($match) === 4 && static::ENCODING_BASE64 === $match[2]){
	...
	public function DKIM_Add($headers_line, $subject, $body){
		...
		$parsedHeaders = array();
		$headerLineIndex = 0;
		$headerLineCount = sizeof($headerLines);
		...
		$copiedHeaderFields = '';
		if($this->DKIM_copyHeaderFields && sizeof($copiedHeaders) > 0){
